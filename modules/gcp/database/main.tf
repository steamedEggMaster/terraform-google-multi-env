locals {
  ...
  // MySQL 전용 binary log 옵션 (PostgreSQL에는 영향 없음)
...
resource "google_sql_database_instance" "default" {
  ...
  database_version = (
    can(regex("^\\d", var.database_version)) ? format("POSTGRES_%s", var.database_version)
    : can(regex("^POSTGRES_", upper(var.database_version))) ? upper(var.database_version)
    : can(regex("^MYSQL_", upper(var.database_version))) ? upper(var.database_version)
    : format("MYSQL_%s", var.database_version)
  )
...
resource "google_sql_user" "default" {
  ...
  project  = var.project_id
  instance = google_sql_database_instance.default.name
  host     = var.database_engine == "MYSQL" ? var.user_host : null
